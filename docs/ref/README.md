...
esflow 按"类"组织的 API 参考。每页一类，写明职责、定义、字段/方法、用法示例与相关类。
...
| [`JobState`](JobState.md) | `esflow.state` | 事件折叠状态：`JobState` / `RunState` / `NodeStatus` / `JobStatus` / `apply_event` |
| [`CheckResult`](CheckResult.md) | `esflow.check` | 启动预检结果 + `pass_check` 聚合 + `FlowCheckError` |
| [`FlowLoadError`](FlowLoadError.md) | `esflow.loader` | flow 目录加载或 DAG 校验失败异常 |
| `load_flow` | `esflow.loader` | 加载 flow 目录，展开静态副本，返回 `(FlowDefine, runs, node_classes)` |
